...
# lpf: bluring -> paraların yüzey ayrıntılarını azaltma
...
# binary threshold -> vadi ile tepeler arasındaki farkı iyice arttırıyoruz.
...
# nesneler birbirlerine yapıştığı için küçültüp geri büyültelim
# açılma -> erozyon ve genisleme
...
# gürültü azaldı ama hala coinler arası köprü var
# nesneler arası distance bulalım
dist_transform = cv2.distanceTransform(opening, cv2.DIST_L2, 5)
plt.figure(), plt.imshow(dist_transform, cmap = "gray"), plt.axis("off")

#öndeki resmi bulmak için nesneleri küçülteceğiz.
ret, sure_foreground = cv2.threshold(dist_transform, 0.4*np.max(dist_transform), 255, 0)
plt.figure(), plt.imshow(sure_foreground, cmap = "gray"), plt.axis("off") # adacıkları bulduk

# arka plan için nesneleri büyültelim
